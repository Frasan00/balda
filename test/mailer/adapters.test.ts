...
describe("Template Adapters", () => {
    ...
    it("should render simple template", async () => {
      await mailer.send((builder) =>
        builder
          ...
          .template("<h1>Hello {{name}}!</h1>", { name: "World" }),
      );
    });
    ...
    it("should handle conditionals", async () => {
      ...
      await mailer.send((builder) =>
        builder
          ...
          .template(templateStr, { isActive: true }),
      );
    });
    ...
    it("should register and use partials", async () => {
      ...
      await mailer.send((builder) =>
        builder
          ...
          .template("{{> header}}<p>Content</p>", { title: "My Title" }),
      );
    });
  });
    ...
    it("should work with custom options", async () => {
      ...
      await customMailer.send((builder) =>
        builder
          ...
          .template("<h1>[?= name ?]</h1>", { name: "Custom" }),
      );
    });
  });
    ...
    it("should leave unmatched variables unchanged", async () => {
      await mailer.send((builder) =>
        builder
          ...
          .template("<p>{{name}} {{missing}}</p>", { name: "John" }),
      );
    });
  ...
  describe("Adapter Comparison", () => {
    it("should produce consistent output across adapters", async () => {
      ...
      await handlebarsMailer.send((builder) =>
        builder
          .to("test@example.com")
          .subject("{{title}}")
    ...
    it("should handle very long templates", async () => {
      ...
      await mailer.send((builder) =>
        builder
          ...
          .template(longTemplate, { content: "Test" }),
      );
    });
    ...
    it("should handle templates with complex HTML", async () => {
      ...
      await mailer.send((builder) =>
        builder
          ...
          .template(complexTemplate, { title: "Title", content: "Content" }),
      );
    });
  });
